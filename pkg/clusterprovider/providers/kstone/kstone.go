...
var kstoneETCDOperatorClient dynamic.Interface
...
type KstoneProviderConfig struct {
	Master         string `json:"master"`
	BarerToken     string `json:"token"`
	ClientCertData string `json:"client_cert_data"`
	ClientKeyData  string `json:"client_key_data"`
	Kubeconfig     string `json:"kubeconfig"`
}

// Init inits kstoneETCDOperatorClient
func Init(cfg interface{}) error {
	c, ok := cfg.(*KstoneProviderConfig)
	if !ok {
		return fmt.Errorf("unknown type for kstone provider init")
	}
	...
	kstoneETCDOperatorClient = client
	return nil
}
...
func (c *EtcdClusterKstone) Create() error {
	...
	_, err = kstoneETCDOperatorClient.Resource(etcdRes).
		Namespace(c.cluster.Namespace).
...
func (c *EtcdClusterKstone) Update() error {
	etcdRes := schema.GroupVersionResource{Group: "etcd.tkestack.io", Version: "v1alpha1", Resource: "etcdclusters"}
	etcd, err := kstoneETCDOperatorClient.Resource(etcdRes).
		Namespace(c.cluster.Namespace).
	...
	_, updateErr := kstoneETCDOperatorClient.Resource(etcdRes).
		Namespace(c.cluster.Namespace).
