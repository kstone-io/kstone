...
func handleHostPath(ctx context.Context, kubecli kubernetes.Interface, endpoints []string, clientTLSSecret, basicAuthSecret, namespace string, insecureSkipVerify bool, isPeriodic bool, maxBackup int) (bs *api.BackupStatus, err error) {
	...
	rev, etcdVersion, now, err := bm.SaveSnap(ctx, s.Path, true) //gyf todo to check
	if err != nil {
